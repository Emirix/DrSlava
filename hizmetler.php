    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            <?php foreach ($config["services"] as $service):
                ...
                $cat = getConfigField($service, 'category');
                $title = getConfigField($service, 'title');
                $desc = getConfigField($service, 'desc');
                $price = getConfigField($service, 'price');
                ?>
                <article
                    class="bg-white rounded-3xl overflow-hidden group luxury-shadow hover:-translate-y-2 transition-all duration-500 animate-fade-in flex flex-col h-full">
                    <div class="h-64 overflow-hidden relative">
                        <img src="<?php echo $service["image"]; ?>" alt="<?php echo $title; ?>"
                            class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
                            width="800" height="600" loading="lazy" />
                        <div
                            class="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-4 py-1 rounded-full text-xs font-medium tracking-wider text-nude-500">
                            <?php echo $price; ?>
                        </div>
                    </div>
                    <div class="p-8 space-y-4 flex flex-col flex-grow">
                        <span
                            class="text-[10px] uppercase tracking-[0.2em] text-nude-400 font-semibold"><?php echo $cat; ?></span>
                        ...
                        <a href="iletisim"
                            class="block w-full text-center py-4 border border-nude-200 rounded-2xl text-sm font-medium hover:bg-nude-400 hover:text-white hover:border-nude-400 transition-all duration-300">
                            <?php echo $lang == "tr"
                                ? "İletişime Geç"
                                : "Get in Touch"; ?>
                        </a>
